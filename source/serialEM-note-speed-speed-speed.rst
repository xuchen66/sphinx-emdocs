...
In our experience, it is best to save on local SSD first, and align frames using framewatcher (IMOD program) and let the program move the processed raw frames and aligned output average to network drive. This way, not only the loal SSD drive will never be filled, but also the network activities on the LAN can be spreat out more evenly. Data collection doesn't need to wait any extra time for saving to be finished onto network drive. 
